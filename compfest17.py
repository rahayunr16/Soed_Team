...
Dataset yang akan digunakan mencakup:
- Parameter Meteorologis: Suhu udara, kecepatan angin, durasi sinar matahari, radiasi gelombang pendek, dan evapotranspirasi.
- Data Waktu: Tahun, bulan, dan hari.
- Data Historis Konsumsi Energi: Konsumsi listrik harian dalam GWh.
...
- Mengimport semua library yang diperlukan untuk analisis data:
  ...
  - Untuk manipulasi data: pandas, numpy
  - Untuk visualisasi: matplotlib.pyplot, seaborn
  - Untuk analisis statistik : scipy.stats, statsmodels.formula.api, statsmodels.api
  - Untuk machine learning : scikit-learn (termasuk modul metrics, preprocessing, ensemble, model_selection, linear_model), xgboost
  - Untuk interaksi file dan penyimpanan model: google.colab.files, joblib, io, pickle, os.
  - Untuk drive eksternal: google.colab.drive.  
- Menyiapkan modul-modul spesifik untuk analisis seperti chi2_contingency dari scipy.stats dan combinations dari itertools.
- Melakukan konfigurasi dasar yang mungkin diperlukan untuk visualisasi atau pengolahan data (implisit dari impor library).
"""
...
df_train1['year'] = df_train1['year'].astype('object')
...
* Distribusi `year`: Frekuensi data per tahun (dari 2014 hingga 2021) juga terlihat sangat konsisten dan merata, yaitu sekitar 1460 data per tahun.
...
* Distribusi `year`: Data terbagi secara sempurna dan merata di antara tahun 2014 hingga 2021, dengan masing-masing tahun mewakili 12.5%.
...
categorical_vars = ['cluster_id', 'year', 'month', 'day_of_week']
...
* `year` menunjukkan ada perbedaan konsumsi yang signifikan secara keseluruhan, tetapi ini terutama didorong oleh perbedaan antara 2020 dengan beberapa tahun sebelumnya, tidak semua tahun berbeda satu sama lain.
...
categorical_columns_to_encode = ['cluster_id', 'year', 'month', 'day_of_week']
...
print(f"\n--- Tampilan Awal Variabel Fitur (X) ---")
print(f"\nJumlah fitur (kolom X) setelah menghapus 'date' dan target: {X.shape[1]}")
print(f"Kolom X: {X.columns.tolist()}")


print(f"\n--- Tampilan Awal Variabel Target (y) ---")
print(f"Ukuran variabel target (y): {y.shape}")
...
actual_numerical_features_to_scale = [col for col in numerical_features_to_scale if col in X_train.columns]

if not actual_numerical_features_to_scale:
    print("\nPERHATIAN: Tidak ada kolom numerik yang teridentifikasi untuk standarisasi.")
    print("Pastikan daftar 'numerical_features_to_scale' sudah benar dan kolomnya ada di X_train.")
    print("Tipe data kolom X_train:")
    print(X_train.dtypes)
else:
    print(f"\nKolom numerik yang akan distandarisasi: {actual_numerical_features_to_scale}")

scaler = StandardScaler()

X_train[actual_numerical_features_to_scale] = scaler.fit_transform(X_train[actual_numerical_features_to_scale])
X_test[actual_numerical_features_to_scale] = scaler.transform(X_test[actual_numerical_features_to_scale])

print("\n--- X_train setelah Standarisasi (5 baris pertama, fokus pada kolom yang discale) ---")
print(X_train[actual_numerical_features_to_scale].head())

print("\n--- X_test setelah Standarisasi (5 baris pertama, fokus pada kolom yang discale) ---")
print(X_test[actual_numerical_features_to_scale].head())
...
df_test1['year'] = df_test1['year'].astype('object')
...
df_test1_encoded[actual_numerical_features_to_scale] = scaler.transform(df_test1_encoded[actual_numerical_features_to_scale])

# Menghapus kolom date agar sesuai dengan data train
df_test_final = df_test1_encoded.drop(columns=['date'], errors='ignore')
...
# Muat dataset test
df_subb = pd.read_csv('submission.csv')
print("\nDataset 'submission.csv' berhasil dimuat.")
print("\n--- DATASET SUBMISSION OVERVIEW ---")
print("=" * 50)
print(f"Shape: {df_subb.shape[0]} rows, {df_subb.shape[1]} columns")
print(f"Memory usage: {df_subb.memory_usage(deep=True).sum() / 1024:.2f} KB")
print("\nColumns:")
for i, col in enumerate(df_subb.columns, 1):
    print(f"  {i:2d}. {col}")
